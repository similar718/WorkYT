...
package com.yt.bleandnfc.ui.zxing.wifi;
...
public final class WifiConfigManager extends AsyncTask<WifiParsedResult, Object, Object> {
  ...
  protected Object doInBackground(WifiParsedResult... args) {
    ...
    if (!wifiManager.isWifiEnabled()) {
      LogUtlis.i(TAG, "Enabling wi-fi...");
      if (wifiManager.setWifiEnabled(true)) {
        LogUtlis.i(TAG, "Wi-fi enabled");
      } else {
        LogUtlis.w(TAG, "Wi-fi could not be enabled!");
        return null;
      }
      ...
      while (!wifiManager.isWifiEnabled()) {
        if (count >= 10) {
          LogUtlis.i(TAG, "Took too long to enable wi-fi, quitting");
          return null;
        }
        LogUtlis.i(TAG, "Still waiting for wi-fi to enable...");
        try {
          Thread.sleep(1000L);
    ...
    try {
      ...
      LogUtlis.w(TAG, "Bad network type");
      return null;
    }
  ...
  private static void updateNetwork(WifiManager wifiManager, WifiConfiguration config) {
    ...
    if (foundNetworkID != null) {
      LogUtlis.i(TAG, "Removing old configuration for network " + config.SSID);
    ...
    if (networkId >= 0) {
      ...
      if (wifiManager.enableNetwork(networkId, true)) {
        LogUtlis.i(TAG, "Associating to network " + config.SSID);
        wifiManager.saveConfiguration();
      } else {
        LogUtlis.w(TAG, "Failed to enable network " + config.SSID);
      }
    } else {
      LogUtlis.w(TAG, "Unable to add network " + config.SSID);
    }
  }

...
import com.yt.bleandnfc.ui.zxing.wifi.WifiConfigManager;
...
public final class WifiResultHandler extends ResultHandler {
  ...
  public void handleButtonPress(int index) {
    if (index == 0) {
      ...
      if (wifiManager == null) {
        LogUtlis.w(TAG, "No WifiManager available from device");
        return;
      }
      ...
      activity.runOnUiThread(new Runnable() {
        @Override
        public void run() {
          ToastUtils.showText(activity.getApplicationContext(),R.string.wifi_changing_network);
        }
      });

...
package com.yt.bleandnfc.ui.zxing.camera.open;
...
public final class OpenCameraInterface {
  ...
  public static OpenCamera open(int cameraId) {
    ...
    if (numCameras == 0) {
      LogUtlis.w(TAG, "No cameras!");
      return null;
    }
    if (cameraId >= numCameras) {
      LogUtlis.w(TAG, "Requested camera does not exist: " + cameraId);
      return null;
    }
    ...
    if (cameraId <= NO_REQUESTED_CAMERA) {
      ...
      if (cameraId == numCameras) {
        LogUtlis.i(TAG, "No camera facing " + CameraFacing.BACK + "; returning camera #0");
        cameraId = 0;
      }
    }

    LogUtlis.i(TAG, "Opening camera #" + cameraId);
    Camera.CameraInfo cameraInfo = new Camera.CameraInfo();
    Camera.getCameraInfo(cameraId, cameraInfo);

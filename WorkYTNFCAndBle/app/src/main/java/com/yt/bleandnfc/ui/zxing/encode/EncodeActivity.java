...
package com.yt.bleandnfc.ui.zxing.encode;
...
import com.yt.bleandnfc.ui.zxing.Contents;
import com.yt.bleandnfc.ui.zxing.FinishListener;
import com.yt.bleandnfc.ui.zxing.Intents;
...
public final class EncodeActivity extends Activity {
  ...
  private void share() {
    ...
    if (encoder == null) { // Odd
      LogUtlis.w(TAG, "No existing barcode to send?");
      return;
    }
    ...
    if (!barcodesRoot.exists() && !barcodesRoot.mkdirs()) {
      LogUtlis.w(TAG, "Couldn't make dir " + barcodesRoot);
      showErrorMessage(R.string.msg_unmount_usb);
      return;
    }
    File barcodeFile = new File(barcodesRoot, makeBarcodeFileName(contents) + ".png");
    if (!barcodeFile.delete()) {
      LogUtlis.w(TAG, "Could not delete " + barcodeFile);
      // continue anyway
    }
    try (FileOutputStream fos = new FileOutputStream(barcodeFile)) {
      bitmap.compress(Bitmap.CompressFormat.PNG, 0, fos);
    } catch (IOException ioe) {
      LogUtlis.w(TAG, "Couldn't access file " + barcodeFile + " due to " + ioe);
      showErrorMessage(R.string.msg_unmount_usb);
      return;
    }
  ...
  protected void onResume() {
    ...
    try {
      ...
      if (bitmap == null) {
        LogUtlis.w(TAG, "Could not encode barcode");
      ...
      LogUtlis.w(TAG, "Could not encode barcode", e);
      showErrorMessage(R.string.msg_encode_contents_failed);
      qrCodeEncoder = null;
    }
  }

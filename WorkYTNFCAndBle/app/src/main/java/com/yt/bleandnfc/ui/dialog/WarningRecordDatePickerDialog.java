...
import com.yt.bleandnfc.ui.view.PickerView;
...
public class WarningRecordDatePickerDialog implements View.OnClickListener, PickerView.OnSelectListener {
    ...
    private TextView mTvHourUnit, mTvMinuteUnit,mTvYaer,mTvMonth,mTvDay,mTvSure;
    ...
    // 默认不显示年份
    private boolean mIsShowYear = false;

    /**
     * 时间选择结果回调接口
     */
    public interface Callback {
        void onTimeSelected(int year,int month,long timestamp);
    }
     ...
     */
    public WarningRecordDatePickerDialog(Context context, Callback callback, String beginDateStr, String endDateStr) {
        this(context, callback, TimeUtil.str2Long(beginDateStr, true),
                TimeUtil.str2Long(endDateStr, true));
    }
     ...
     */
    public WarningRecordDatePickerDialog(Context context, Callback callback, long beginTimestamp, long endTimestamp) {
    ...
    private void initView() {
        mPickerDialog = new Dialog(mContext, R.style.CustomDialog);
        mPickerDialog.requestWindowFeature(Window.FEATURE_NO_TITLE);
        mPickerDialog.setContentView(R.layout.dialog_setting_departure_time);
        ...
        mTvYaer = mPickerDialog.findViewById(R.id.tv_year);
        mTvMonth = mPickerDialog.findViewById(R.id.tv_month);
        mTvDay = mPickerDialog.findViewById(R.id.tv_day);
        mTvSure = mPickerDialog.findViewById(R.id.tv_sure);

        mTvSure.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (mCallback != null) {
                    mCallback.onTimeSelected(mSelectedTime.get(Calendar.YEAR),mSelectedTime.get(Calendar.MONTH),mSelectedTime.getTimeInMillis());
        ...
        setShowYear();
        // 对数据进行赋值处理
        String date = TimeUtil.long2Str(mSelectedTime.getTimeInMillis(), true);
//        mSettingEndTime.setText(date.substring(5,date.length()));
    }
...
//            mTvMinuteUnit.setVisibility(View.VISIBLE);
            mTvYaer.setVisibility(View.VISIBLE);
            mTvMonth.setVisibility(View.VISIBLE);
//            mTvDay.setVisibility(View.VISIBLE);
        } else {
//            mTvHourUnit.setVisibility(View.INVISIBLE);
//            mTvMinuteUnit.setVisibility(View.INVISIBLE);
            mTvYaer.setVisibility(View.INVISIBLE);
            mTvMonth.setVisibility(View.INVISIBLE);
//            mTvDay.setVisibility(View.INVISIBLE);
        }
    }

    private void setShowYear(){
        if (mIsShowYear){
            ...
            mTvYaer.setVisibility(View.GONE);
        }
    }
...
//        String date = TimeUtil.long2Str(mSelectedTime.getTimeInMillis(), true);
//        mSettingEndTime.setText(date.substring(5,date.length()));
//        mDestinationContent.setText(destinationStr);
//        if (!TextUtils.isEmpty(detail)){
////            mDestinationDetail.setText(detail);
//        } else {
////            mDestinationDetail.setText("正在加载数据");
//        }
    }
    ...
    public boolean setSelectedTime(String dateStr, boolean showAnim) {
        return canShow() && !TextUtils.isEmpty(dateStr)
                && setSelectedTime(TimeUtil.str2Long(dateStr, mCanShowPreciseTime), showAnim);
    }

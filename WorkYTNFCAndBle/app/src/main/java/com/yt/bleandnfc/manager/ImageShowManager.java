...
import com.yt.bleandnfc.manager.glide.image.BlurTransformation;
import com.yt.bleandnfc.manager.glide.image.GlideCircleTransform;
...
public class ImageShowManager {
     ...
     * @param imgeview 组件
     */
    public static void showImageView(Context context, int errorimg, Uri url,
                                     ImageView imgeview) {
        Glide.with(context).load(url)// 加载图片
                .error(errorimg)// 设置错误图片
                .crossFade()// 设置淡入淡出效果，默认300ms，可以传参
     ...
     * @param url
     */
    public static void showImageViewGoneNoCahe(Context context, final ImageView imageView, String url) {
        Glide.with(context).load(url).asBitmap()
